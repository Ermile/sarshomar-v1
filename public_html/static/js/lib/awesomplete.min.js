...
{
	var elTags = $('#tag')[0];
	var listTags = new Awesomplete(elTags,
	{
		minChars: 0,
		autoFirst: true,
	...
	$(document).on('input', 'input#tag', function(e)
	{
		fillDataList(this, listTags);
	});
}
 ...
 */
function fillDataList(_this, _target)
{
	// console.log($(_this));
	// get vals for search
	var listName   = $(_this).attr('id');
	...
	}, function(data)
	{
		if(data && data.msg && data.msg.list)
		{
			...
			var test = JSON.parse(list).map(function(i)
			{
				var opt = i.title;
				opt += '<var>' + i.count + '</var>';
				if(i.desc)
				{
					opt += '<span>' + i.desc + '</span>';
				}
				return opt;
			});
			// console.log(list);
			// console.log(test);
			_target.list = test;
			// var awesomplete = new Awesomplete($(_this).parent(), {
			// 	minChars: 1,
			// 	autoFirst: true
			// });
			// new Awesomplete($(_this),{ list: list });
			// $(_this).attr('data-list', list);
			// awesomplete.list = list;


			// console.log(test);
			// // console.log(list);
			// $.each(list, function(_key, _value)
			// {
			// 	var myoption = document.createElement('option');
			// 	myoption.value = _value.title + '<span>' + _value.count +'</span>';
